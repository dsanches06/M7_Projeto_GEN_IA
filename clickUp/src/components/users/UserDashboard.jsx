...
export default function UserDashboard({ user, onBack }) {
  ...
  const stats = [
    ...
    {
      label: "Concluídas",
      value: (user.tasks || []).filter((t) => t.status === "COMPLETED").length,
      icon: completedIcon,
    },
    {
      label: "Bloqueadas",
      value: (user.tasks || []).filter((t) => t.status === "BLOCKED").length,
      icon: taskIcon,
    },
  ];
    ...
    <div className="w-full max-w-7xl mx-auto px-6 py-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        {stats.map(({ label, value, icon }) => (
          <div
            key={label}
            className="flex items-center gap-4 rounded-3xl bg-surface p-2 text-left transition-colors"
          >
            {icon && (
          ...
          {STATUS_COLUMNS.map((s) => (
            ...
            >
              ...
              {byStatus[s].map((t) => (
                ...
                >
                  <p className="text-xs font-medium text-main leading-snug">
                    {t.title}
                  </p>
