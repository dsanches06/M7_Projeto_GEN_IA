...
export class StatisticPageUI {
    ...
    render() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.renderRankingMoreHoursChart(chartsGrid);
            yield this.renderRankingIncreasedHoursChart(chartsGrid);
            yield this.renderRankingAboveAverageChart(chartsGrid);
            yield this.renderStatisticsSummaryChart(chartsGrid);
            yield this.renderRankingChart(chartsGrid);
        });
    }
    /**
  ...
  `;
    ...
    drawBarChart(canvas, labels, data, colors = []) {
        ...
        labels.forEach((label, index) => {
            ...
            // Desenhar barra
            ctx.fillStyle = color;
            this.drawRoundedRect(ctx, 100, y - barHeight / 2 + 10, width, barHeight - 20, 6);
    ...
    drawVerticalBarChart(canvas, labels, data, colors = []) {
        const ctx = canvas.getContext("2d");
        if (!ctx)
            return;
        const padding = 50;
     ...
     */
    drawLegend(ctx, labels, data, colors, startX) {
    ...
    renderChartMessage(chartContainer, message) {
    ...
    renderRankingMoreHoursChart(container) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const chartContainer = this.createChartContainer("📈 Top 3 Usuários por Horas Reais");
            container.appendChild(chartContainer);
            const canvas = this.createCanvas();
            try {
                const ranking = yield this.statisticsService.getRankingMoreHours();
                if (!ranking.length) {
                    this.renderChartMessage(chartContainer, "Nenhum resultado encontrado para ranking de horas.");
                    return;
                }
                ...
                (_a = chartContainer.querySelector(".chart-body")) === null || _a === void 0 ? void 0 : _a.appendChild(canvas);
                this.charts.set("overview", canvas);
            }
            catch (error) {
                console.error("Erro ao carregar ranking de horas:", error);
                this.renderChartMessage(chartContainer, "Erro ao carregar ranking de horas.");
            }
        });
    }
    renderRankingIncreasedHoursChart(container) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const chartContainer = this.createChartContainer("📈 Maior Hora do Dia por Usuário");
            container.appendChild(chartContainer);
            const canvas = this.createCanvas();
            try {
                const ranking = yield this.statisticsService.getRankingIncreasedHours();
                if (!ranking.length) {
                    this.renderChartMessage(chartContainer, "Nenhum resultado encontrado para aumento de horas.");
                    return;
                }
                const selectedEntries = ranking.slice(0, 12);
                const rawDates = Array.from(new Set(selectedEntries.map((entry) => entry.data_dia))).sort((a, b) => new Date(a).getTime() - new Date(b).getTime());
                ...
                selectedEntries.forEach((entry) => {
                    ...
                    userTotalsMap.set(entry.utilizador, ((_a = userTotalsMap.get(entry.utilizador)) !== null && _a !== void 0 ? _a : 0) + horas);
                });
                ...
                const datasets = topUsers.map((utilizador) => ({
                    ...
                    data: rawDates.map((rawDate) => {
                        const entry = selectedEntries.find((item) => item.utilizador === utilizador && item.data_dia === rawDate);
                ...
                this.renderChartMessage(chartContainer, "Erro ao carregar ranking de aumento de horas.");
            }
        });
    }
    /**
     ...
     */
    renderRankingAboveAverageChart(container) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const chartContainer = this.createChartContainer("📊 Projetos Acima da Média de Horas");
            container.appendChild(chartContainer);
            const canvas = this.createCanvas();
            try {
                const ranking = yield this.statisticsService.getRankingAboveAverage();
                if (!ranking.length) {
                    this.renderChartMessage(chartContainer, "Nenhum resultado encontrado para projetos acima da média.");
                    return;
                }
                ...
                if (total === 0) {
                    this.renderChartMessage(chartContainer, "Nenhum valor válido para exibir o gráfico de projetos acima da média.");
                    return;
                }
                ...
                this.renderChartMessage(chartContainer, "Erro ao carregar projetos acima da média.");
            }
        });
    }
    /* Renderiza o gráfico de ranking de projetos acima da média */
    renderRankingChart(container) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const chartContainer = this.createChartContainer("📊 Projetos Acima da Média de Horas");
            container.appendChild(chartContainer);
    ...
    renderStatisticsSummaryChart(container) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const chartContainer = this.createChartContainer("📉 Resumo do Dashboard de Estatísticas");
            container.appendChild(chartContainer);
            const canvas = this.createCanvas();
            try {
                const moreHours = yield this.statisticsService.getRankingMoreHours();
                const increasedHours = yield this.statisticsService.getRankingIncreasedHours();
                const aboveAverage = yield this.statisticsService.getRankingAboveAverage();
                const data = [
                    moreHours.length,
                ...
                if (data.every((value) => value === 0)) {
                    this.renderChartMessage(chartContainer, "Nenhum dado de estatísticas disponível para exibir resumo.");
                    return;
                }
                this.drawDoughnutChart(canvas, ["Top Usuários", "Aumentos de Horas", "Projetos Acima da Média"], data, ["#007bff", "#28a745", "#ffc107"]);
                (_a = chartContainer.querySelector(".chart-body")) === null || _a === void 0 ? void 0 : _a.appendChild(canvas);
                this.charts.set("comparison", canvas);
            }
            catch (error) {
                console.error("Erro ao carregar resumo de estatísticas:", error);
                this.renderChartMessage(chartContainer, "Erro ao carregar resumo de estatísticas.");
            }
        });
    }
    /**
