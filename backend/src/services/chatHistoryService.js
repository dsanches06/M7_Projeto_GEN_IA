...
export const getAllChatHistory = async () => {
  ...
  return rows.map(mapChatHistoryDTOResponse);
};
export const getChatHistoryById = async (id) => {
  const [rows] = await db.query("SELECT * FROM chat_history WHERE id = ?", [id]);
  return rows[0] ? mapChatHistoryDTOResponse(rows[0]) : null;
};
export const getChatHistoryByConversationId = async (conversationId) => {
  const [rows] = await db.query(
    "SELECT * FROM chat_history WHERE conversation_id = ? ORDER BY sent_at ASC", [conversationId]);
  return rows.map(mapChatHistoryDTOResponse);
};
