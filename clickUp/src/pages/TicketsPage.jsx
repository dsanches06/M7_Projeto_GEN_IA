...
export default function TicketsPage() {
  ...
  function cycleSortDir() { setSortDir((d) => d === "NONE" ? "ASC" : d === "ASC" ? "DESC" : "NONE"); }

  // ── Filter + sort ──────────────────────────────────────────────────────────
  const filtered = tickets.filter((t) => {
    const q = search.toLowerCase();
    const matchSearch  = !q || (t.user_report || "").toLowerCase().includes(q) || (t.error_type || "").toLowerCase().includes(q);
  ...
  const filterLabel =
    ...
    filterSev    !== "all" ?
      (filterSev === "critical" ? "Crítica (8-10)" :
       ...
       filterSev === "medium"   ? "Média (3-4)"    : "Baixa (1-2)") :
    "Todos";
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6 animate-fadeInUp">
      ...
      <div className="mb-4 sm:mb-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-main">Tickets</h2>
      ...
      <div className="flex flex-col gap-3 mb-4 sm:mb-6">
        ...
        {/* Stat cards – 2×2 on mobile (same as before), unchanged */}
          ...
          {statCards.map(({ label, value, icon, filter, color }) => {
            ...
            return (
              <button key={label} type="button"
                ...
                className={`flex items-center gap-2 sm:gap-3 rounded-2xl sm:rounded-3xl p-2 text-left transition-colors cursor-pointer hover:bg-surface-2 active:scale-95 ${
                  isSelected ? "bg-surface-2 border border-surface" : "bg-surface"
                }`}
              >
                <span className="flex-shrink-0 w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center"
                  style={{ background: `${color}18`, color }}>
                  {icon}
                </span>
                <div className="min-w-0">
                  <p className="text-[9px] sm:text-[10px] text-muted uppercase tracking-[0.06em] leading-tight">{label}</p>
                  <p className="text-sm sm:text-base font-semibold text-main">{value}</p>
                </div>
              </button>
            );
          })}
        ...
        {/* ── Filter controls – properly aligned on mobile ── */}
        <div className="flex flex-col gap-2">

          {/* Row 1: two dropdowns side by side */}
          <div className="flex gap-2">
            <select
              className="flex-1 h-9 bg-surface border border-surface rounded-lg px-2.5 text-xs text-main focus:outline-none focus:border-[var(--primary)]"
              value={filterSev}
          ...
          {/* Row 2: search input (when visible) + icon buttons right-aligned */}
          <div className="flex items-center gap-2">
            {showSearch && (
              <input autoFocus
                className="h-9 bg-surface border border-surface rounded-lg px-3 text-sm text-main placeholder:text-muted focus:outline-none focus:border-[var(--primary)] flex-1 min-w-0"
            ...
            <span className="hidden sm:inline-flex text-xs text-muted px-3 py-1 rounded-full border border-surface bg-surface-2 whitespace-nowrap">
              {filterLabel}
            </span>
      ...
      {/* ── Content ── */}
      {loading && (
        <div className="text-center py-16 text-muted text-sm">A carregar tickets…</div>
      )}
      ...
      {error && !loading && (
        <div className="text-center py-10">
          <div className="inline-flex flex-col items-center gap-4 rounded-3xl border border-red-600/20 bg-red-600/5 p-8 mx-auto max-w-xl">
            ...
            <p className="text-sm text-red-200">Erro: {error}</p>
            <button type="button" onClick={load}
              className="rounded-full border border-red-500 bg-red-500/10 px-4 py-2 text-sm font-semibold text-red-100">
              Recarregar
            </button>
          </div>
        </div>
        ...
        <>
          ...
          <section className="bg-surface-2 border border-surface rounded-2xl sm:rounded-3xl p-4 sm:p-6 shadow-sm">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between mb-4 sm:mb-6">
              <div>
                <h3 className="text-lg sm:text-xl font-bold text-main">Tickets</h3>
                <p className="text-muted mt-1 text-xs sm:text-sm max-w-2xl">
                  Use o ChatBot 🤖 para criar novos tickets via inteligência artificial.
                </p>
              </div>
