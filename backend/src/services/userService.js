...
export const toggleUserActive = async (id, data) => {
  const [, r] = await db.query("UPDATE users SET active = ? WHERE id = ?", [
    data.active,
    id,
  ]);
  return r.affectedRows;
...
export const getUserStats = async () => {
  const [r1] = await db.query("SELECT COUNT(*) AS tu FROM users");
  const total = parseInt(r1[0]?.tu ?? r1[0]?.count ?? 0);
  const [r2] = await db.query(
    "SELECT COUNT(*) AS au FROM users WHERE active = 1",
  );
  const active = parseInt(r2[0]?.au ?? r2[0]?.count ?? 0);
  const [r3] = await db.query(
    "SELECT COUNT(*) AS iu FROM users WHERE active = 0",
  );
  const inactive = parseInt(r3[0]?.iu ?? r3[0]?.count ?? 0);
