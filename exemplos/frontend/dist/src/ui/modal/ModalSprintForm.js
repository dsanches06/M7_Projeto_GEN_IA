...
import { SprintService, ProjectStatusService } from "../../services/index.js";
...
function setupSprintFormLogic(form, fields, errors, modal, projectId, sprintToEdit) {
        ...
        if (isValid) {
            try {
                ...
                if (sprintToEdit) {
                    ...
                    yield SprintService.updateSprint(sprintData.id, sprintData);
                    showInfoBanner(`INFO: O sprint ${name} foi atualizado com sucesso.`, "info-banner");
                }
                else {
                    ...
                    yield SprintService.createSprint(sprintData);
                    showInfoBanner(`INFO: O sprint ${name} foi criado com sucesso.`, "info-banner");
                }
                ...
                if (dashboardElement) {
                    ...
                    yield loadSprintsPage(sprints);
                }
                modal.remove();
            }
            catch (error) {
                const action = sprintToEdit ? "atualizar" : "criar";
                showInfoBanner(`ERRO: Não foi possível ${action} o sprint ${name}.`, "error-banner");
                console.error(`Erro ao ${action} sprint:`, error);
            }
        }
        else {
            showInfoBanner(`ERRO: O sprint não foi criado. Verifique os erros no formulário.`, "error-banner");
        }
    });
}
/**
 ...
 */
export function renderSprintModal(projectId, sprintToEdit) {
    return __awaiter(this, void 0, void 0, function* () {
        const modal = createSection("modalSprintForm");
        modal.classList.add("modal");
        const content = createSection("modalSprintContent");
        content.classList.add("modal-content");
        const closeBtn = document.createElement("span");
        closeBtn.classList.add("close");
        closeBtn.innerHTML = "&times;";
        closeBtn.onclick = () => modal.remove();
        const titleHeading = createHeadingTitle("h2", sprintToEdit ? "Editar Sprint" : "Adicionar Sprint");
        const form = createForm("formSprint");
        // Criação dos campos usando a função auxiliar
        const nameData = createInputGroup("Nome do Sprint", "sprintNameInput", "text", "inserir o nome do sprint");
        if (sprintToEdit) {
            nameData.input.value = sprintToEdit.name;
        }
        ...
        const startDateData = createInputGroup("Data de Início", "sprintStartDateInput", "date", "selecionar data de início");
        if (sprintToEdit) {
            startDateData.input.value = sprintToEdit.start_date;
        }
        const endDateData = createInputGroup("Data de Fim", "sprintEndDateInput", "date", "selecionar data de fim");
        if (sprintToEdit) {
            endDateData.input.value = sprintToEdit.end_date;
        }
        ...
        try {
            const statuses = yield ProjectStatusService.getProjectStatuses();
        ...
        const submitBtn = createButton("button", sprintToEdit ? "Atualizar Sprint" : "Criar Sprint", "submit");
        form.append(nameData.section, descriptionData.section, startDateData.section, endDateData.section, statusGroup, submitBtn);
