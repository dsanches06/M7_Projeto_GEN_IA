...
export const getAllNotifications = async () => {
  ...
  return rows.map(mapNotificationDTOResponse);
};
export const getNotificationById = async (id) => {
  const [rows] = await db.query("SELECT * FROM notification WHERE id = ?", [id]);
  return rows[0] ? mapNotificationDTOResponse(rows[0]) : null;
};
export const getNotificationsByUser = async (userId) => {
  const [rows] = await db.query("SELECT * FROM notification WHERE user_id = ? ORDER BY sent_at DESC", [userId]);
  return rows.map(mapNotificationDTOResponse);
};
export const getUnreadNotifications = async (userId) => {
  const [rows] = await db.query(
    "SELECT * FROM notification WHERE user_id = ? AND is_read = FALSE ORDER BY sent_at DESC", [userId]);
  return rows.map(mapNotificationDTOResponse);
};
...
export const updateNotification = async (id, data) => {
  const [, result] = await db.query(
    ...
    [data.message, data.is_read, id]);
  return result.affectedRows ?? 0;
};
export const toggleReadStatus = async (id, is_read) => {
  const [, result] = await db.query("UPDATE notification SET is_read = ? WHERE id = ?", [is_read, id]);
  return result.affectedRows ?? 0;
};
