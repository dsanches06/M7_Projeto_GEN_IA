...
// Cartão de ticket com barra de severidade, tipo de erro e estado
export default function TicketCard({ ticket, onSelect, delay = 0 }) {
  const sev     = ticket.severity || 1;
  // Estilos de cor e label baseados na severidade (1-10)
  const sevStyle = getSeverityStyle(sev);
  // Configuração de cor e label para o tipo de erro
  const errCfg  =
    ERROR_TYPE_CONFIG[(ticket.error_type || "").toLowerCase()] ||
    ERROR_TYPE_CONFIG.other;
  // Configuração de cor e label para o estado do ticket
  const statCfg =
  ...
  return (
    ...
    >
      {/* Linha superior: ID, tipo de erro e estado */}
      ...
      {/* Barra de severidade com percentagem proporcional */}
      <div className="mb-3">
      ...
      {/* Sugestão de correcção (visível apenas se existir) */}
